...
const trainerTutorials = [
  ['1. Salve seus códigos', 'No cadastro, guarde o código de acesso curto e a chave de recuperação. Compartilhe somente o código público FC-XXXXXXXX.'],
...
export default function HelpPage() {
  return <main className="min-h-screen bg-background px-4 py-10"><div className="mx-auto max-w-5xl space-y-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl border p-5"><KeyRound className="mb-2 h-5 w-5 text-amber-600" /><h2 className="font-bold">Perdeu o acesso?</h2><p className="mt-2 text-sm text-muted-foreground">Personal: use nome, código público e chave de recuperação em <Link href="/recover" className="text-primary underline">Recuperar acesso</Link>. Aluno: peça ao personal um novo código.</p></div><div className="rounded-xl border p-5"><ShieldCheck className="mb-2 h-5 w-5 text-emerald-600" /><h2 className="font-bold">Boas práticas</h2><p className="mt-2 text-sm text-muted-foreground">Não envie o código de acesso ou a chave de recuperação em grupos. Troque o código se houver suspeita de acesso.</p></div></section>
