      ...
      <section className="dk-hero-panel p-6 sm:p-8">
        <div className="relative z-10 flex flex-col items-start justify-between gap-7 sm:flex-row sm:items-center">
          <div className="flex items-center gap-4">
            <Avatar className="size-24 border-4 border-white/15">{student.avatar_url && <AvatarImage src={student.avatar_url} alt={`Avatar de ${student.full_name}`} />}<AvatarFallback className="bg-brand-accent text-2xl font-black text-black">{student.full_name.split(' ').map((name) => name[0]).join('').slice(0, 2).toUpperCase()}</AvatarFallback></Avatar>
            <div><div className="mb-3 flex flex-wrap items-center gap-2"><span className="dk-kicker text-brand-accent">Atleta D KONG</span><Badge className="dk-volt-chip border-0">{student.status === 'active' ? 'Ativo' : 'Arquivado'}</Badge></div><h2 className="dk-display text-3xl sm:text-4xl">{student.full_name}</h2><p className="mt-3 flex items-center gap-1.5 text-sm text-white/48"><Activity className="size-3.5 text-brand-accent" /> {student.goal || 'Sem objetivo'}</p></div>
          </div>
          <div className="flex w-full flex-wrap gap-2 sm:w-auto"><Button variant="outline" className="flex-1 border-white/15 bg-white/10 text-white hover:bg-white/20 hover:text-white" onClick={() => router.push(`/messages?contactId=${params.id}`)}><Mail className="mr-2 size-4" /> Mensagem</Button><Button variant="outline" className="flex-1 border-white/15 bg-white/10 text-white hover:bg-white/20 hover:text-white" onClick={() => void rotateStudentCode()} disabled={rotatingCode}>{rotatingCode ? <Loader2 className="mr-2 size-4 animate-spin" /> : <KeyRound className="mr-2 size-4" />} Novo código</Button><Button className="flex-1 bg-brand-accent text-black hover:bg-brand-accent-hover" onClick={openEdit}><Edit3 className="mr-2 size-4" /> Editar</Button></div>
        </div>
      </section>

      <Card className={newAccessCode ? 'border-brand-accent-deep/35 bg-brand-accent/12' : undefined}>
