      ...
      <section className="dk-hero-panel p-6 sm:p-8">
        <div className="relative z-10 flex flex-col items-start justify-between gap-7 sm:flex-row sm:items-center">
          <div className="flex items-center gap-4">
            ...
            <div><div className="mb-3 flex flex-wrap items-center gap-2"><span className="dk-kicker rounded-md bg-white px-2 py-1 text-black">Atleta G KONG</span><Badge className="dk-volt-chip border-0">{student.status === 'active' ? 'Ativo' : 'Arquivado'}</Badge></div><h2 className="dk-display text-3xl sm:text-4xl">{student.full_name}</h2><p className="mt-3 flex items-center gap-1.5 text-sm text-white/70"><Activity className="size-3.5 text-[#c9ff32]" /> {student.goal || 'Sem objetivo'}</p></div>
          </div>
