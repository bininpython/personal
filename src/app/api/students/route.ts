...
export async function POST(request: Request) {
  try {
    ...
    if (authError || !authData.user) {
      return NextResponse.json({ error: 'Erro ao criar autenticação do aluno' }, { status: 400 });
    }

    const id = authData.user.id; // Use the UUID generated by Supabase Auth
    ...
    if (insertError) {
      return NextResponse.json({ error: 'Erro ao cadastrar aluno no banco' }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      student: { id, name: data.full_name },
    });
  } catch (error) {
