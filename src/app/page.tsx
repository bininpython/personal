    ...
    <div className="min-h-screen overflow-hidden bg-[#f6f6f1] text-[#0a0a0a]">
        ...
        <section className="relative mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-12 sm:px-8 sm:pb-24 sm:pt-20 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:gap-16">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-9 flex flex-wrap gap-x-6 gap-y-3">
              {highlights.map((item) => (
                <span key={item} className="inline-flex items-center gap-2 text-xs font-semibold text-black/70">
          ...
          <div className="relative z-10 mx-auto w-full max-w-[540px]">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-black/10 bg-white p-5 shadow-[0_40px_100px_rgba(0,0,0,0.14)] sm:p-8">
              ...
              <div className="flex items-center justify-between border-b border-black/10 pb-5">
                <span className="text-[0.68rem] font-black uppercase tracking-[0.2em] text-black/65">The training OS</span>
                <Sparkles className="size-4 text-[#7cae00]" />
              </div>
              <div className="mt-6 rounded-[1.6rem] bg-[#090a08] p-4 text-white shadow-2xl sm:p-5">
                <div className="flex items-center justify-between"><div><p className="text-[0.6rem] font-black uppercase tracking-[0.2em] text-[#c9ff32]">Central de performance</p><p className="mt-1 text-lg font-black">Bom dia, Marina.</p></div><span className="flex size-10 items-center justify-center rounded-full bg-[#c9ff32] text-black"><Activity className="size-4" /></span></div>
                <div className="mt-5 grid grid-cols-2 gap-2">
                  <div className="rounded-2xl border border-white/12 bg-white/[0.06] p-3"><p className="text-[0.58rem] font-black uppercase tracking-[0.14em] text-white/65">Alunos ativos</p><p className="mt-3 text-3xl font-black tracking-[-0.06em]">08</p></div>
                  <div className="rounded-2xl bg-[#c9ff32] p-3 text-black"><p className="text-[0.58rem] font-black uppercase tracking-[0.14em] text-black/65">Constância</p><p className="mt-3 text-3xl font-black tracking-[-0.06em]">86%</p></div>
                </div>
                <div className="mt-2 rounded-2xl border border-white/12 bg-white/[0.06] p-3"><div className="flex items-center justify-between"><div><p className="text-[0.58rem] font-black uppercase tracking-[0.14em] text-white/65">Próxima ação</p><p className="mt-1 text-sm font-black">Renovar ficha de Lucas</p></div><ArrowRight className="size-4 text-[#c9ff32]" /></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full w-[78%] rounded-full bg-[#c9ff32]" /></div></div>
                <div className="mt-4 space-y-2">{[['Ana Souza', '92%'], ['Rafael Lima', '84%'], ['Lucas Dias', '61%']].map(([name, progress], index) => <div key={name} className="flex items-center gap-3 rounded-xl border border-white/8 px-3 py-2"><span className="flex size-7 items-center justify-center rounded-full bg-white/10 text-[10px] font-black">0{index + 1}</span><p className="flex-1 text-xs font-bold">{name}</p><span className="text-xs font-black text-[#c9ff32]">{progress}</span></div>)}</div>
              </div>
              <div className="grid grid-cols-3 gap-2 border-t border-black/10 pt-5 text-center">
                {[['254+', 'exercícios'], ['3', 'passos'], ['6', 'dígitos']].map(([value, label]) => (
                  <div key={label}>
                    <p className="text-lg font-black">{value}</p>
                    <p className="text-[0.68rem] font-bold uppercase tracking-[0.1em] text-black/65">{label}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="absolute -bottom-6 -right-3 rounded-2xl bg-black px-5 py-4 text-white shadow-2xl sm:-right-8">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-xs font-bold">Operação rápida</p><p className="text-[0.68rem] text-white/70">do cadastro ao treino</p></div>
              </div>
            </div>
          </div>
        </section>
              ...
              {capabilities.map((item) => (
                <article key={item.number} className="group border-b border-white/12 py-9 lg:border-b-0 lg:border-r lg:px-8 lg:py-12 first:lg:pl-0 last:lg:border-r-0 last:lg:pr-0">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-white/65">/{item.number}</span>
                  ...
                  <p className="mt-4 max-w-sm text-sm leading-6 text-white/70">{item.description}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="px-5 py-20 sm:px-8 sm:py-28">
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.86fr_1.14fr] lg:gap-24">
            <div>
              ...
              <p className="mt-6 max-w-lg text-base leading-7 text-black/70">Um fluxo pensado para funcionar na academia, entre uma série e outra — rápido, legível e sem códigos desnecessários.</p>
            </div>
            <div className="space-y-4">
              {[
                ...
                <div key={step.tag} className="grid gap-5 rounded-3xl border border-black/10 bg-white p-6 shadow-sm sm:grid-cols-[56px_1fr_auto] sm:items-center">
                  ...
                  <div>
                    <p className="text-[0.68rem] font-black uppercase tracking-[0.16em] text-black/65">{step.tag}</p>
                    <h3 className="mt-1 text-lg font-black">{step.title}</h3>
                    <p className="mt-1 text-sm leading-6 text-black/70">{step.text}</p>
                  </div>
                  <span className="font-mono text-sm text-black/60">0{index + 1}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer className="px-5 py-8 sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 border-t border-black/10 pt-7 sm:flex-row sm:items-center sm:justify-between">
          <BrandMark compact />
          <div className="flex gap-5 text-xs font-semibold text-black/65">
          ...
          <p className="text-xs text-black/60">© {new Date().getFullYear()} D KONG</p>
        </div>
      </footer>
    </div>
  );
