...
import { formatDateInSaoPaulo } from '@/lib/time/sao-paulo';
        ...
        <div className="space-y-3">
          {history.map((workout) => (
            <Card key={workout.id}><CardContent className="p-5">
              <div className="mb-3 flex items-start justify-between gap-3"><div><p className="font-semibold">{workout.name}</p><div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Calendar className="size-3" />{formatDateInSaoPaulo(workout.date)}</span><span className="flex items-center gap-1"><Clock className="size-3" />{formatDuration(workout.durationSeconds)}</span></div></div><Badge className="bg-emerald-500/10 text-emerald-600"><CheckCircle2 className="mr-1 size-3" /> {workout.completion}%</Badge></div>
