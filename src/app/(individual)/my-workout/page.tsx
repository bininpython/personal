...
export default function IndividualWorkoutPage() {
  ...
  function selectDay(dayId: string) {
    if (runningDayId && runningDayId !== dayId && completedSets.size > 0) {
      toast.info('Finalize ou reinicie o treino atual antes de trocar de ficha.');
      return;
    }
  ...
  async function completeWorkout() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Não foi possível concluir seu treino.');
      clearLocalProgress();
      if (data.alreadyCompletedToday) toast.info('Você já concluiu o treino de hoje.');
      else if (data.alreadyCompletedThisWeek) toast.info('Esta ficha já atingiu a meta da semana.');
      else toast.success('Treino concluído! O próximo da rotação já foi preparado.');
      await loadPlan(true);
    } catch (completionError) {
      toast.error(completionError instanceof Error ? completionError.message : 'Não foi possível concluir seu treino.');
    } finally {
      setSubmitting(false);
    }
  }

  if (loading) return <div className="flex min-h-[60vh] items-center justify-center text-muted-foreground"><Loader2 className="mr-2 size-5 animate-spin" /> Preparando seu treino...</div>;
  if (error && !plan) return <div className="mx-auto max-w-md py-20 text-center"><Dumbbell className="mx-auto size-12 text-muted-foreground/40" /><h1 className="mt-4 text-xl font-black">Não foi possível abrir seu treino</h1><p className="mt-2 text-sm text-muted-foreground">{error}</p><Button className="mt-5" onClick={() => void loadPlan()}><RefreshCw className="mr-2 size-4" /> Tentar novamente</Button></div>;
  if (!plan || !activeDay) return <div className="mx-auto max-w-md py-20 text-center"><Dumbbell className="mx-auto size-12 text-muted-foreground/40" /><h1 className="mt-4 text-xl font-black">Monte sua primeira ficha</h1><p className="mt-2 text-sm text-muted-foreground">Depois de salvar a ficha, o treino executável aparecerá aqui.</p><Button nativeButton={false} className="mt-5" render={<Link href="/my-exercises" />}><Plus className="mr-2 size-4" /> Montar ficha</Button></div>;

  return (
    <div className="mx-auto max-w-5xl space-y-5 pb-12 animate-fade-in">
      <section className="dk-hero-panel p-6 sm:p-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="dk-kicker text-[#c9ff32]">Meu treino individual</p><h1 className="dk-display mt-4 text-3xl sm:text-4xl">{plan.name}</h1><p className="mt-2 text-sm text-white/70">{plan.goal}</p></div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3"><div className="rounded-2xl border border-white/12 bg-white/[0.06] p-4"><Target className="size-4 text-[#c9ff32]" /><p className="mt-2 text-lg font-black">{plan.daysPerWeek}x</p><p className="text-[10px] text-white/60">meta semanal</p></div><div className="rounded-2xl bg-[#c9ff32] p-4 text-black"><CheckCircle2 className="size-4" /><p className="mt-2 text-lg font-black">{plan.week.completed}/{plan.week.target}</p><p className="text-[10px] text-black/60">concluídos</p></div><div className="rounded-2xl border border-white/12 bg-white/[0.06] p-4"><Dumbbell className="size-4 text-[#c9ff32]" /><p className="mt-2 text-lg font-black">{suggestedDay?.label || activeDay.label}</p><p className="text-[10px] text-white/60">próximo treino</p></div></div>
      </section>

      <Card className={plan.week.isComplete ? 'border-[#9fdb00]/30 bg-[#c9ff32]/10' : ''}><CardContent className="p-4 sm:p-5"><div className="flex items-center justify-between gap-4"><div><p className="font-bold">{plan.week.isComplete ? 'Meta semanal concluída' : 'Progresso da semana'}</p><p className="text-xs text-muted-foreground">A sequência continua sempre na ordem {plan.days.map((day) => day.label).join(' → ')} → {plan.days[0]?.label}.</p></div><strong className="text-2xl text-[#668f00]">{Math.min(100, Math.round((plan.week.completed / Math.max(1, plan.week.target)) * 100))}%</strong></div><Progress className="mt-3" value={(plan.week.completed / Math.max(1, plan.week.target)) * 100} /></CardContent></Card>

      <div className="flex gap-2 overflow-x-auto pb-1">{plan.days.map((day) => <button type="button" key={day.id} onClick={() => selectDay(day.id)} className={`relative min-w-[135px] rounded-xl border px-4 py-3 text-left ${activeDay.id === day.id ? 'border-black bg-black text-[#c9ff32] dark:border-[#c9ff32] dark:bg-[#c9ff32] dark:text-black' : day.completedThisWeek ? 'border-[#9fdb00]/40 bg-[#c9ff32]/10' : 'border-border bg-card'}`}>{day.completedThisWeek && <CheckCircle2 className="absolute right-2 top-2 size-4" />}<span className="block text-[10px] font-black uppercase opacity-70">Treino {day.label}</span><span className="mt-1 block truncate font-bold">{day.name}</span><span className="mt-1 block text-[10px] opacity-70">{day.weeklyCompletions}/{day.weeklyAllowance} na semana</span></button>)}</div>

      {lockedUntilTomorrow && <Card className="border-[#9fdb00]/30 bg-[#c9ff32]/10"><CardContent className="flex gap-3 p-5"><CalendarClock className="mt-0.5 size-5 shrink-0 text-[#668f00]" /><div><p className="font-bold">Próximo da rotação: Treino {activeDay.label}</p><p className="mt-1 text-sm text-muted-foreground">Você já concluiu o treino de hoje. O próximo fica disponível amanhã, mas pode ser consultado agora.</p></div></CardContent></Card>}

      {lockedByRotation && !lockedUntilTomorrow && !activeDay.completedThisWeek && <Card className="border-amber-500/30 bg-amber-500/10"><CardContent className="flex gap-3 p-5"><CalendarClock className="mt-0.5 size-5 shrink-0 text-amber-600" /><div><p className="font-bold">Ficha disponível somente para consulta</p><p className="mt-1 text-sm text-muted-foreground">Para manter sua progressão, conclua primeiro o treino destacado na sequência.</p></div></CardContent></Card>}

      {!isRunning && !unavailable && <Card className="overflow-hidden border-[#9fdb00]/30"><CardContent className="p-6 text-center sm:p-8"><div className="mx-auto flex size-14 items-center justify-center rounded-full bg-[#c9ff32] text-black"><Play className="ml-1 size-6" /></div><p className="mt-4 text-[10px] font-black uppercase tracking-[0.18em] text-[#668f00]">Próximo da sequência</p><h2 className="mt-2 text-2xl font-black">Treino {activeDay.label} · {activeDay.name}</h2><p className="mt-2 text-sm text-muted-foreground">{activeDay.exercises.length} exercícios · {totalSets} séries. O tempo começa quando você tocar no botão.</p><Button className="mt-5 h-12 bg-black px-8 text-white dark:bg-[#c9ff32] dark:text-black" onClick={startWorkout}><Play className="mr-2 size-4" /> Iniciar treino {activeDay.label}</Button></CardContent></Card>}

      {isRunning && <Card><CardContent className="p-4 sm:p-5"><div className="flex items-center justify-between gap-4"><div><p className="font-bold">Treino {activeDay.label} em andamento</p><p className="text-xs text-muted-foreground">{completedCount} de {totalSets} séries concluídas</p></div><strong className="text-2xl text-[#668f00]">{progress}%</strong></div><Progress className="mt-3" value={progress} />{progress > 0 && <Button variant="ghost" size="sm" className="mt-2 text-xs text-muted-foreground" onClick={clearLocalProgress}><RotateCcw className="mr-1 size-3" /> Reiniciar treino</Button>}</CardContent></Card>}
      ...
      {isRunning && progress === 100 && <Card className="border-[#9fdb00]/40 bg-[#c9ff32]/10"><CardContent className="p-6 text-center"><CheckCircle2 className="mx-auto size-10 text-[#668f00]" /><h2 className="mt-3 text-xl font-black">Todas as séries concluídas</h2><p className="mt-1 text-sm text-muted-foreground">Avalie o treino e salve para liberar o próximo da rotação.</p><div className="mt-4 flex justify-center gap-1">{[1, 2, 3, 4, 5].map((value) => <button type="button" key={value} onClick={() => setRating(value)} aria-label={`${value} estrela(s)`} className="flex size-11 items-center justify-center rounded-full"><Star className={`size-6 ${value <= rating ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground/40'}`} /></button>)}</div><Textarea className="mx-auto mt-3 max-w-md" rows={2} maxLength={1000} value={feedback} onChange={(event) => setFeedback(event.target.value)} placeholder="Como foi o treino? (opcional)" /><Button className="mt-4 bg-black text-white dark:bg-[#c9ff32] dark:text-black" disabled={submitting} onClick={() => void completeWorkout()}>{submitting && <Loader2 className="mr-2 size-4 animate-spin" />} Concluir e salvar treino</Button></CardContent></Card>}
