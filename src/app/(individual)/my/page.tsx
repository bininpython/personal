      ...
      <section className="dk-hero-panel overflow-hidden p-6 sm:p-8 lg:p-10">
        <div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
          <div>
            ...
            <div className="mt-7 flex flex-wrap gap-3">{activePlan ? <Button nativeButton={false} render={<Link href="/my-workout" />} className="h-12 rounded-full bg-[#c9ff32] px-6 text-black hover:bg-[#d5ff60]"><PlayCircle className="mr-2 size-4" /> {activeWorkout?.week.completedToday ? 'Ver próxima ficha' : 'Iniciar ficha do dia'}</Button> : <Button nativeButton={false} render={<Link href="/my-exercises" />} className="h-12 rounded-full bg-[#c9ff32] px-6 text-black hover:bg-[#d5ff60]"><BookOpen className="mr-2 size-4" /> Montar primeira ficha</Button>}<Button nativeButton={false} variant="outline" render={<Link href="/my-plans" />} className="h-12 rounded-full border-white/20 bg-white/5 px-6 text-white hover:bg-white/10 hover:text-white"><FileText className="mr-2 size-4" /> Minhas fichas</Button></div>
          </div>
