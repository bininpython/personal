  ...
  return (
    <div className="space-y-7 pb-10 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Button nativeButton={false} variant="ghost" render={<Link href="/my-plans" />} className="font-bold text-foreground hover:bg-accent">
          <ArrowLeft className="mr-2 size-4 text-[#c9ff32]" /> Voltar para Minhas Fichas
        </Button>
        <div className="flex flex-wrap gap-2">
          <Button nativeButton={false} variant="outline" render={<Link href={`/my-exercises?planId=${plan.id}`} />}>
          ...
          {isDemoUser(user?.id) ? (
            ...
            <Button variant="outline" onClick={() => void downloadPdfFile(`/api/individual/workout-plans/${plan.id}/pdf`, `${plan.name}.pdf`)}>
