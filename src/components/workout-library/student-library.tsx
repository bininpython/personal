...
import { BookOpenCheck, Download, Loader2, PlayCircle, ShieldCheck } from 'lucide-react';
...
export function StudentLibrary() {
  ...
  return (
    <div className="space-y-7 pb-10 animate-fade-in">
      ...
      {loading ? <div className="flex justify-center py-20 text-muted-foreground"><Loader2 className="mr-2 size-5 animate-spin" /> Carregando suas fichas...</div> : error ? <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-8 text-center text-sm text-destructive">{error}</div> : assignments.length === 0 ? <div className="rounded-3xl border border-dashed p-12 text-center"><BookOpenCheck className="mx-auto size-12 text-muted-foreground/30" /><h2 className="mt-5 text-xl font-black">Nenhuma ficha da Biblioteca foi enviada</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Quando seu personal escolher um programa pré-pronto para você, ele aparecerá aqui automaticamente.</p></div> : <div className="grid gap-5 md:grid-cols-2">{assignments.map((plan) => <Card key={plan.id} className="overflow-hidden border-[#9fdb00]/35"><div className="h-1.5 bg-[#c9ff32]" /><CardHeader><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#668b00]">Enviada pelo personal</p><CardTitle className="text-xl">{plan.name}</CardTitle><p className="text-sm text-muted-foreground">{plan.goal}</p></CardHeader><CardContent className="space-y-4"><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-muted/60 p-3"><p className="font-black">{plan.workoutDayCount}</p><p className="text-[9px] uppercase text-muted-foreground">Fichas</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="font-black">{plan.exerciseCount}</p><p className="text-[9px] uppercase text-muted-foreground">Exercícios</p></div><div className="rounded-xl bg-muted/60 p-3"><p className="font-black">{plan.daysPerWeek}x</p><p className="text-[9px] uppercase text-muted-foreground">Semana</p></div></div><div className="grid grid-cols-2 gap-2"><Button nativeButton={false} render={<Link href="/workout" />}><PlayCircle className="mr-2 size-4" /> Iniciar treino</Button><Button nativeButton={false} variant="outline" render={<a href={`/api/workout-plans/${plan.id}/pdf`} download />}><Download className="mr-2 size-4" /> PDF</Button></div></CardContent></Card>)}</div>}
    </div>
  );
}
