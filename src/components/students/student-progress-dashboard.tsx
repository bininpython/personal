    ...
    <div className="space-y-6">
      ...
      {data.assessments.length > 0 ? (
        <div className="grid gap-6 xl:grid-cols-2">
          <Card className="print:break-inside-avoid">
            <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Scale className="size-5 text-brand-accent-strong" /> Peso e massa muscular</CardTitle></CardHeader>
            <CardContent><div className="h-72"><ResponsiveContainer width="100%" height="100%"><LineChart data={data.assessments}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="dateLabel" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 11 }} domain={['auto', 'auto']} /><Tooltip /><Legend /><Line type="monotone" dataKey="weight" name="Peso (kg)" stroke={BRAND_COLORS.accentStrong} strokeWidth={3} connectNulls /><Line type="monotone" dataKey="muscleMass" name="Massa muscular (kg)" stroke="#111111" strokeWidth={3} connectNulls /></LineChart></ResponsiveContainer></div></CardContent>
          </Card>
          <Card className="print:break-inside-avoid">
