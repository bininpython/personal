...
export function WorkoutBuilder({ mode }: { mode: 'trainer' | 'individual' }) {
  ...
  const loadEditablePlan = useCallback(async (planId: string) => {
    ...
    try {
      const response = await fetch(isIndividual ? `/api/individual/workout-plans/${planId}` : `/api/workout-plans/${planId}`, { cache: 'no-store' });
      ...
      router.push(isIndividual ? '/my-plans' : '/workouts');
    } finally {
  ...
  }, [isIndividual, loadEditablePlan, loadMuscle]);
  ...
  async function submitPlan() {
    if ((!isIndividual && !selectedStudentId) || !planName.trim()) {
      toast.error(isIndividual ? 'Informe o nome da ficha.' : 'Selecione o aluno e informe o nome da ficha.');
      return;
    }
    ...
    try {
      const endpoint = isIndividual
        ? (editingPlanId ? `/api/individual/workout-plans/${editingPlanId}` : '/api/individual/workout-plans')
        : (editingPlanId ? `/api/workout-plans/${editingPlanId}` : '/api/workout-plans');
      const response = await fetch(endpoint, {
        method: editingPlanId ? 'PATCH' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...(!isIndividual && !editingPlanId ? { studentId: selectedStudentId } : {}),
          name: planName,
          goal,
          daysPerWeek,
          endDate,
      ...
      toast.success(editingPlanId ? 'Nova versão da ficha publicada.' : isIndividual ? 'Sua ficha foi criada e ativada.' : 'Ficha publicada para o aluno.');
      setIsSaveOpen(false);
      router.push(isIndividual ? '/my-plans' : '/workouts');
    } catch (error) {
    ...
    <div ref={builderRef} className="space-y-6 pb-28 animate-fade-in xl:pb-10">
      <PageHeader
        ...
        actions={
          <Button onClick={openSaveDialog} disabled={totalSelected === 0} className="hidden h-11 xl:inline-flex">
            ...
            {editingPlanId ? 'Salvar alterações' : isIndividual ? 'Criar minha ficha' : 'Publicar ficha'} ({totalSelected})
          </Button>
        }
      />
      ...
      <div className="flex items-start gap-2 rounded-xl border border-info/20 bg-info-wash p-3 text-sm text-muted-foreground">
        ...
        <span>
          ...
          A prescrição deve respeitar limitações, experiência e execução segura do praticante.
        </span>
      </div>
        ...
        <div className="mx-auto flex max-w-[1600px] items-center gap-2 px-4 py-3">
          ...
          {mobileStep < LAST_MOBILE_STEP ? (
            ...
            <Button type="button" onClick={openSaveDialog} disabled={totalSelected === 0}>
              ...
              {editingPlanId ? 'Salvar' : isIndividual ? 'Criar ficha' : 'Publicar'}
            </Button>
          )}
        </div>
      </div>
      ...
      <Dialog open={isSaveOpen} onOpenChange={setIsSaveOpen}>
        <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingPlanId ? 'Salvar nova versão da ficha' : isIndividual ? 'Criar minha ficha' : 'Publicar ficha para o aluno'}</DialogTitle>
            <DialogDescription>
              {editingPlanId
                ? isIndividual ? 'A versão atual permanece no histórico e a nova passa a ser sua ficha ativa.' : 'A versão atual permanece no histórico e a nova passa a ser exibida para o aluno.'
                : isIndividual ? 'A ficha será ativada na sua área e poderá ser baixada em PDF.' : 'Ao publicar, esta ficha passa a ser o treino ativo do aluno e a anterior é arquivada.'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            {!isIndividual && <div className="space-y-1.5">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => void submitPlan()} disabled={submitting || (!isIndividual && students.length === 0)}>
              {submitting && <Loader2 className="mr-2 size-4 animate-spin" />}
              {editingPlanId ? 'Salvar nova versão' : isIndividual ? 'Criar e ativar' : 'Publicar agora'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
