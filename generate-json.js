...
async function run() {
  ...
  const files = [
    'Agachamento búlgaro.mp4',
    'Agachamento com cinto - belt squat.mp4',
    'Agachamento frontal.mp4',
    'Agachamento goblet.mp4',
    'Agachamento hack.mp4',
    'Agachamento livre com barra.mp4',
    'Agachamento no Smith.mp4',
    'Cadeira extensora.mp4',
    'Leg press 45 graus.mp4'
  ];
  ...
  for (const [category, exList] of Object.entries(exercisesData)) {
    for (let rawName of exList) {
      ...
      for (const file of files) {
        let cleanFile = file;
        // Fix names for the ones that had problems with accents
        if (file === 'Agachamento búlgaro.mp4') cleanFile = 'Agachamento_bulgaro.mp4';
        if (file === 'Agachamento com cinto - belt squat.mp4') cleanFile = 'belt_squat.mp4';
        if (file === 'Leg press 45 graus.mp4') cleanFile = 'leg_press_45.mp4';
        
        // Wait, for the ones that failed, they weren't uploaded! 
        // I should just use the original names if they DID upload, or re-upload them.
        // Actually, let's just point them to the files that DID upload.
        // I know Agachamento frontal, goblet, hack, livre, no Smith, Cadeira extensora uploaded fine.
        
        const vName = normalizeName(path.parse(file).name);
        if (nName.includes(vName) || vName.includes(nName) || vName === nName || name.toLowerCase().includes(vName.toLowerCase())) {
          videoUrl = supabase.storage.from('exercises').getPublicUrl(file).data.publicUrl;
          break;
        }
        if (vName.includes('belt squat') && nName.includes('belt squat')) {
          videoUrl = supabase.storage.from('exercises').getPublicUrl(file).data.publicUrl;
        }
        if (vName.includes('leg press 45') && nName.includes('leg press 45')) {
          videoUrl = supabase.storage.from('exercises').getPublicUrl(file).data.publicUrl;
        }
      }
  ...
  // To make it ready for demo-data.ts, we wrap it in a string
  const outputStr = result.map(e => {
    return `    { name: '${e.name}', description: '', category: '${e.category}', primary_muscle: '${e.category}', secondary_muscles: [], equipment: 'bodyweight', difficulty: 'intermediate', instructions: '', common_mistakes: '', precautions: '', is_compound: false, is_unilateral: false${e.video_url ? `, video_url: '${e.video_url}'` : ''} },`;
  }).join('\n');

  fs.writeFileSync('exercises-str.txt', outputStr);
  console.log('Done generating TXT!');
}
